...
class AddGuessTest(unittest.TestCase):
    ...
    def test_add_guest_id_nul(self):
        '''参数id为空'''
        payload = {'eid':'','realname':'jerry','phone':'18069446230'}
        r = requests.get(self.base_url,data = payload)
        self.result = r.json()
        self.assertEqual(self.result['status'],10021)
        self.assertEqual(self.result['message'],'parameter error')

    def test_add_guest_realname_null(self):
        '''参数realname为空'''
        payload = {'eid':'4','realname':'','phone':'18069446230','email':'347862945@qq.com'}
        r = requests.get(self.base_url,data = payload)
        self.result = r.json()
        self.assertEqual(self.result['status'],10021)
        self.assertEqual(self.result['message'],'parameter error')
